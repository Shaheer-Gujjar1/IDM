...
- **In-App Secure Software Updater**: Integrated `tauri-plugin-updater` and `@tauri-apps/plugin-updater`. Configured signed updater endpoints in `src-tauri/tauri.conf.json` (`latest.json`), registered updater plugin in Rust backend (`src-tauri/src/lib.rs`), and added a "Check for Updates" UI section under Settings with live progress reporting.
