...
- **Linux Package Post-Uninstall Cleanup & Ghost Icon Prevention**: Integrated `postrm` maintainer scripts into `.deb` and `postRemoveScript` into `.rpm` packages. Automatically purges leftover user-level autostart `.desktop` files (`~/.config/autostart/impressive-download-manager.desktop`) and browser native host manifests across standard (`/home/*`, `/root`) and OSTree/immutable distros (`/var/home/*`) upon package uninstallation to prevent ghost launcher icons in Deepin, GNOME, and KDE. Removed unconditional autostart creation from `setup()` to prevent the Tauri updater flow from recreating the entry after removal.
