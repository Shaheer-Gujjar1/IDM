...
- **Multi-Platform OS-Specific Auto-Updates**: Updated `scripts/generate_latest_json.js` to register distinct target platform keys (`windows-x86_64`, `linux-x86_64-deb`, `linux-x86_64-rpm`, `linux-x86_64-appimage`, `darwin-x86_64`, `darwin-aarch64`). Users on Windows receive `.exe` installers, users on Debian/Ubuntu receive lightweight `.deb` updates (~8 MB), users on Fedora/RHEL receive `.rpm`, and macOS users receive native `.dmg`/`.app` bundles.
